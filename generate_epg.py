...
def add_cctv_id_aliases(root):
    ...
    for ch in channels:
        ...
        for alias in aliases_id:
            if alias in existing_ids:
                # 如果别名已存在但没有节目，则把原频道节目复制过去
                if alias not in prog_map:
                    for prog in progs:
    ...
    print(f"  ✓ 央视 ID 别名: 新增 {added_channels} 个频道, 复制 {added_progs} 条节目")
